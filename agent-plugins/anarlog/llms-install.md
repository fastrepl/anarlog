# Install the BlackMushi MCP server

1. Confirm the user has BlackMushi Pro and has enabled **Settings → Developers → Cloud API & Connectors**.
...
4. If Cloud returns no meetings the user expects, check whether `anarlog` is on `PATH` with `anarlog --version` and fill the gap with `anarlog --json`. If the CLI is missing, ask the user to install it from **BlackMushi → Settings → Developers** or follow <https://docs.anarlog.so/installation>. Do not install software or search the filesystem without permission.
5. Never query or modify BlackMushi's SQLite database directly.
