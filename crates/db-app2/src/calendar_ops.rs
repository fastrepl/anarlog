...
use crate::{Calendar, UpsertCalendar};

pub async fn get_calendar(pool: &SqlitePool, id: &str) -> Result<Option<Calendar>, sqlx::Error> {
...
pub async fn list_calendars(pool: &SqlitePool) -> Result<Vec<Calendar>, sqlx::Error> {
    sqlx::query_as::<_, Calendar>("SELECT * FROM calendars ORDER BY name, id")
        .fetch_all(pool)
        .await
}
...
pub async fn insert_calendar_if_missing(
    ...
    let result = sqlx::query(
        "INSERT INTO calendars \
         (id, tracking_id_calendar, name, enabled, provider, source, color, connection_id, created_at, updated_at) \
