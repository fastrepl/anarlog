...
async fn finalize_shared_attachment(
    ...
    let object_digest = state
        .storage
        .object_digest(BUCKET, &object_key, expected_size)
        .await
        .map_err(|error| {
            tracing::warn!(%error, "shared attachment object digest verification failed");
            SyncError::SharedAttachmentServiceUnavailable
        })?;
    if object_digest.size_bytes != expected_size || object_digest.sha256 != expected_sha256 {
        return Err(SyncError::SharedAttachmentConflict);
    }
