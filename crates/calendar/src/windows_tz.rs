// CLDR windowsZones.xml default (territory=001) Windows ID -> IANA mappings.
...
mod tests {
    ...
    fn mapped_iana_names_parse_in_chrono_tz() {
        for windows_tz in [
            ...
            let iana = windows_tz_to_iana(windows_tz).expect(windows_tz);
            iana.parse::<chrono_tz::Tz>()
                .unwrap_or_else(|_| panic!("{windows_tz} -> {iana}"));
        }
    }
}
