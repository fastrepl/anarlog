...
use crate::adapter::{BatchSttAdapter, append_provider_param, is_char_proxy};
...
mod tests {
    use super::*;
    use crate::{DeepgramAdapter, CharAdapter, OpenAIAdapter};
    use hypr_language::{ISO639, Language};

    #[test]
    fn injects_provider_for_char_proxy() {
        let client = BatchClient::<CharAdapter>::builder()
            .api_base("https://api.char.com/stt")
            .api_key("test")
            .build();

        assert!(client.api_base.contains("provider=char"));
    }
    ...
    #[test]
    fn injects_provider_for_direct_provider_adapter_on_char_proxy() {
    ...
    fn rewrites_existing_provider_for_direct_provider_adapter() {
        let client = BatchClient::<OpenAIAdapter>::builder()
            .api_base("https://api.char.com/stt?provider=char&model=whisper-1")
            .api_key("test")
            .build();
