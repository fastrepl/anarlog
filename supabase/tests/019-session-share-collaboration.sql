  ...
  $$
    ...
    where schemaname = 'public'
      and indexname in (
        ...
        'session_access_events_collaboration_share_created_idx'
      )
  $$,
  array[3::bigint],
  'Collaboration request and notification feeds have targeted lookup indexes'
);
  ...
  (
    ...
    where namespace.nspname = 'public'
      and proc.proname in (
        ...
        'list_my_session_collaboration_events'
      )
  ),
...
select results_eq(
  ...
  $$,
  array[3::bigint],
  'A named Viewer can list the active comment feed'
);
...
select results_eq(
  $$
    ...
    from public.list_my_session_collaboration_events()
  $$,
  $$
    values (
      array[
  ...
  $$,
  'Owners see only actionable collaboration event categories'
);
...
select ok(
  exists (
    select 1
    from public.list_my_session_collaboration_events()
    where event_type in ('comment_created', 'comment_deleted')
  )
    and not exists (
      select 1
      from public.list_my_session_collaboration_events()
  ...
  'Collaborators see other comment activity but not notifications for their own actions'
);
...
select results_eq(
  $$select count(*) from public.list_my_session_collaboration_events()$$,
  array[0::bigint],
  'Ungranted users cannot enumerate collaboration activity'
);

select ok(
  (
    select proc.proargnames[4:8] = array[
      'event_id',
      'share_id',
      'event_type',
      'related_entity_id',
    ...
    join pg_namespace as namespace
      on namespace.oid = proc.pronamespace
    where namespace.nspname = 'public'
      and proc.proname = 'list_my_session_collaboration_events'
  ),
  'Collaboration activity exposes only opaque identifiers, type, and time'
);
...
select results_eq(
  $$select count(*) from public.list_my_session_collaboration_events()$$,
  array[0::bigint],
  'Unavailable source workspaces terminate collaborator activity access'
);
