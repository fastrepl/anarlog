...
CREATE INDEX session_access_events_collaboration_share_created_idx
  ON public.session_access_events(share_id, created_at DESC, id DESC)
  WHERE event_type IN (
    'request_created',
    'invitation_accepted',
    'comment_created',
...
CREATE OR REPLACE FUNCTION private.list_session_share_comments(
  ...
  p_after_comment_id uuid DEFAULT NULL,
...
BEGIN
  IF (p_after_created_at IS NULL) <> (p_after_comment_id IS NULL) THEN
    ...
    WHERE comment.share_id = p_share_id
      ...
      AND (
        ...
        OR (comment.created_at, comment.id)
          > (p_after_created_at, p_after_comment_id)
      )
    ORDER BY
      CASE WHEN p_after_created_at IS NULL THEN comment.created_at END DESC,
      CASE WHEN p_after_created_at IS NULL THEN comment.id END DESC,
      CASE WHEN p_after_created_at IS NOT NULL THEN comment.created_at END,
      CASE WHEN p_after_created_at IS NOT NULL THEN comment.id END
    LIMIT v_limit
  ) AS selected
...
BEGIN
  ...
  SELECT
    ...
    request.created_at,
    request.reviewed_at
  FROM public.session_access_requests AS request
  WHERE request.share_id = p_share_id
    AND request.requester_user_id = v_actor_id
...
CREATE OR REPLACE FUNCTION private.list_my_session_collaboration_events(
  p_after_created_at timestamptz DEFAULT NULL,
...
BEGIN
  IF (p_after_created_at IS NULL) <> (p_after_event_id IS NULL) THEN
    RAISE EXCEPTION 'invalid collaboration event cursor'
      USING ERRCODE = '22023';
  END IF;
  ...
  FROM (
    WITH managed_shares AS (
      ...
      WHERE event.subject_user_id = v_actor_id
        ...
        AND (
          ...
          OR (event.created_at, event.id)
            > (p_after_created_at, p_after_event_id)
        )
      ...
      SELECT
        ...
        event.related_entity_id,
        event.created_at
      FROM managed_shares AS managed_share
      JOIN public.session_access_events AS event
        ON event.share_id = managed_share.id
      WHERE event.event_type IN (
        ...
        AND event.actor_user_id IS DISTINCT FROM v_actor_id
        AND (
          p_after_created_at IS NULL
          OR (event.created_at, event.id)
            > (p_after_created_at, p_after_event_id)
      ...
      JOIN public.workspaces AS workspace
        ON workspace.id = share.workspace_id
      JOIN public.session_access_events AS event
        ON event.share_id = access_grant.share_id
      WHERE access_grant.grantee_user_id = v_actor_id
        AND access_grant.revoked_at IS NULL
        AND share.deleted_at IS NULL
        AND workspace.deleted_at IS NULL
        AND event.event_type IN ('comment_created', 'comment_deleted')
        AND event.actor_user_id IS DISTINCT FROM v_actor_id
        AND (
          p_after_created_at IS NULL
          OR (event.created_at, event.id)
    ...
    SELECT collaboration_event.*
    FROM collaboration_events AS collaboration_event
    ORDER BY
      CASE WHEN p_after_created_at IS NULL
        THEN collaboration_event.created_at
      END DESC,
      CASE WHEN p_after_created_at IS NULL
        THEN collaboration_event.id
      END DESC,
      CASE WHEN p_after_created_at IS NOT NULL
        THEN collaboration_event.created_at
      END,
      CASE WHEN p_after_created_at IS NOT NULL
        THEN collaboration_event.id
...
REVOKE ALL ON FUNCTION private.inspect_my_session_access_invitation(uuid, text)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.list_my_session_collaboration_events(
  timestamptz,
  uuid,
  integer
) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION private.inspect_my_session_access_invitation(uuid, text)
  TO authenticated;
GRANT EXECUTE ON FUNCTION private.list_my_session_collaboration_events(
  timestamptz,
  uuid,
  integer
) TO authenticated;
...
CREATE OR REPLACE FUNCTION public.list_my_session_collaboration_events(
  p_after_created_at timestamptz DEFAULT NULL,
  p_after_event_id uuid DEFAULT NULL,
  p_limit integer DEFAULT 100
)
...
AS $$
  SELECT *
  FROM private.list_my_session_collaboration_events(
    p_after_created_at,
    p_after_event_id,
    p_limit
  );
...
REVOKE ALL ON FUNCTION public.inspect_my_session_access_invitation(uuid, text)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.list_my_session_collaboration_events(
  timestamptz,
  uuid,
  integer
) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.inspect_my_session_access_invitation(uuid, text)
  TO authenticated;
GRANT EXECUTE ON FUNCTION public.list_my_session_collaboration_events(
  timestamptz,
  uuid,
  integer
) TO authenticated;
