...
function mapSessionRows(rows: SessionSearchSqlRow[]): SearchDocument[] {
  return rows.map((row) => {
    ...
    const meetingChatContent = mergeContent(
      parseStringArray(row.meeting_chats_json).map(flattenMeetingChatDocument),
    );
    ...
    return {
      ...
      content: createSessionSearchableContent({
        ...
        meeting_chat_content: meetingChatContent,
        transcript,
      }),
...
function flattenMeetingChatDocument(value: string): string {
