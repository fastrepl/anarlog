...
it("shows a retryable error when onboarding cannot be persisted", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "Open BlackMushi" }));

  expect(
    (
      screen.getByRole("button", {
        name: "Open BlackMushi",
      }) as HTMLButtonElement
    ).disabled,
  ).toBe(true);
  await waitFor(() => {
    expect(screen.getByRole("alert").textContent).toBe(
      "Couldn't open BlackMushi. Please try again.",
    );
  });
  expect(
    (
...
it("ignores concurrent finish attempts", async () => {
  ...
  render(<FinalSection onContinue={onContinue} />);
  const button = screen.getByRole("button", { name: "Open BlackMushi" });
