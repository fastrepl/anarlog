...
export async function provisionMissingWorkspaceKeys(
  ...
  const missingWorkspaces = credentials.workspaces.filter(
    (workspace) =>
      workspace.kind === "shared" && !activeWorkspaceIds.has(workspace.id),
  );
  ...
  for (const workspace of missingWorkspaces) {
    ...
    const recipients: Array<{ userId: string; publicKey: string }> = [];
  ...
  return provisioned ? ("provisioned" as const) : ("waiting" as const);
}
