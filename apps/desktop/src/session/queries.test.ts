...
describe("session SQLite operations", () => {
  ...
  it("reads the latest raw note after earlier writes for the session", async () => {
    let releaseFirstWrite: (() => void) | undefined;
    const firstWrite = new Promise<number[]>((resolve) => {
      releaseFirstWrite = () => resolve([1]);
    });
    const order: string[] = [];

    mocks.executeTransaction
      .mockImplementationOnce(() => {
        order.push("initial-write");
        return firstWrite;
      })
      .mockImplementationOnce(() => {
        order.push("append-write");
        return Promise.resolve([1]);
      });
    mocks.execute.mockImplementationOnce(() => {
      order.push("append-read");
    ...
    const queuedUpdate = updateSession("session-1", {
    ...
    const queuedAppend = appendSessionRawNote("session-1", updater);
    ...
    await expect(queuedAppend).resolves.toBe(true);

    expect(order).toEqual(["initial-write", "append-read", "append-write"]);
    expect(updater).toHaveBeenCalledWith(
      '{"type":"doc","content":[{"type":"paragraph"}]}',
    );
    const appendStatement = mocks.executeTransaction.mock.calls[1][0][0];
    expect(appendStatement.params).toContain(
      '{"type":"doc","content":[{"type":"paragraph"},{"type":"paragraph"}]}',
    );
  });
  ...
  it.each([
    ["null", null],
    ["unchanged content", '{"type":"doc"}'],
  ])(
    "does not write when the raw note updater returns %s",
    async (_, result) => {
      mocks.execute.mockResolvedValueOnce([
        {
          raw_body: '{"type":"doc"}',
      ...
      await expect(
        appendSessionRawNote("session-1", () => result),
      ).resolves.toBe(false);
  ...
  it("converts imported Markdown before updating the raw note", async () => {
    mocks.execute.mockResolvedValueOnce([
      { raw_body: "Hello", raw_body_format: "markdown" },
    ]);
    const updater = vi.fn((rawMd: string) => rawMd);

    await expect(appendSessionRawNote("session-1", updater)).resolves.toBe(
      false,
    );

    const rawMd = updater.mock.calls[0][0];
    expect(JSON.parse(rawMd)).toMatchObject({ type: "doc" });
    expect(mocks.executeTransaction).not.toHaveBeenCalled();
  });
