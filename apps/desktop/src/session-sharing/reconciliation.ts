...
export async function loadManagedShareProjection(
  ...
  const body = addSharedAttachmentIds(source.body, localToShared);
  return {
    source,
    body,
...
export async function reconcileManagedSessionShareSnapshot(input: {
  ...
  if (await shouldDeferForActiveEditor(input)) return "deferred";
  ...
  if (
    ...
    await writeSyncState({ ...state, status: "conflict" });
    return "conflict";
  }
...
async function importSnapshot(input: {
  ...
  try {
    localBody = restoreLocalAttachmentIds(snapshot.body, localToShared);
  } catch {
    const state = await loadSessionShareSyncState(
      viewerUserId,
      snapshot.shareId,
    );
    if (state) await writeSyncState({ ...state, status: "conflict" });
    return "conflict";
  }
  ...
  const outcome = await enqueueDatabaseWrite(
    ...
    async () => {
      if (await shouldDeferForActiveEditor(input)) return "deferred" as const;
...
async function bestEffortAcknowledge(input: {
  ...
  if (await shouldDeferForActiveEditor(input)) return;
  await input
    .acknowledge(input.snapshot.shareId, input.snapshot.contentRevision)
    .catch(() => undefined);
}
...
async function writeSyncState(state: SessionShareSyncState): Promise<void> {
