...
Use BlackMushi's typed, read-only interfaces for meeting data. Do not use `find`,
`grep`, `rg`, filesystem crawling, or direct SQLite queries to find or read
meetings.

Prefer the BlackMushi MCP tools when they are available:
...
If MCP is unavailable, use the BlackMushi CLI with `--json`:
...
The CLI discovers BlackMushi's database from the platform application-data
