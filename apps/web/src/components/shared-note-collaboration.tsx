...
export function SharedNoteCollaboration({
  ...
  const commentsQuery = useQuery({
    ...
    queryFn: async () => {
      const result = await listSharedNoteComments({ data: shareId });
      if (result.status !== "ready") {
        throw new Error("comments unavailable");
      }
      return result.comments;
    },
    enabled: signedIn && hasAuthenticatedAccess,
    retry: false,
  });
  ...
  const commentForm = useForm({
    ...
    onSubmit: async ({ formApi, value }) => {
      await createMutation.mutateAsync(value.body.trim());
      formApi.reset();
    },
  });
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-5">
        ...
        {hasAuthenticatedAccess && commentsQuery.isSuccess && (
      ...
      {!signedIn ? (
        ...
        <>
          {hasAuthenticatedAccess ? (
            <CommentList
              comments={comments}
              error={commentsQuery.isError || deleteMutation.isError}
                  ...
                  <>
                    ...
                    <div className="mt-3 flex items-center justify-between gap-4">
                      ...
                      <button
                        ...
                        disabled={
                          createMutation.isPending || !field.state.value.trim()
                        }
                      >
...
function ManagerRequests({
  ...
  onLoadEarlier: () => void;
  onReview: (requestId: string, decision: "approved" | "denied") => void;
  pendingRequestId: string | null;
  requests: Array<{ entryId: string; userEmail: string }>;
}) {
  if (loading) {
    return null;
  }
              ...
              >
                ...
                <div className="mt-3 flex gap-2 sm:mt-0">
                  <button
                    ...
                    onClick={() => onReview(request.entryId, "denied")}
                  >
                  ...
                  <button
                    ...
                    disabled={pendingRequestId !== null}
                    onClick={() => onReview(request.entryId, "approved")}
                  >
                    {pending ? (
                      <LoaderCircleIcon
