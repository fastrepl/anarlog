...
const reviewSharedNoteAccessRequestInputSchema = z
  .object({
...
const reviewedAccessRequestRowSchema = z.discriminatedUnion("status", [
  z
    .object({
      ...
      capability: z.literal("commenter"),
    })
    .strict(),
  z
    .object({
...
export const listSharedNoteComments = createServerFn({ method: "GET" })
  ...
  .handler(
    async ({
      ...
      | { status: "ready"; comments: SharedNoteComment[] }
      | { status: "unavailable" }
      | { status: "error" }
    > => {
      setPrivateShareResponseHeaders();
      ...
      try {
        return {
          status: "ready",
          comments: data.map(parseSharedNoteComment),
        };
      } catch {
        return { status: "error" };
      }
...
export const createSharedNoteComment = createServerFn({ method: "POST" })
  ...
  .handler(
    ...
    > => {
      setPrivateShareResponseHeaders();
      if (new TextEncoder().encode(data.body).byteLength > 16_384) {
        return { status: "unavailable" };
      }
  ...
  .handler(async ({ data: input }): Promise<SharedNoteOperationStatus> => {
    ...
    const { data, error } = await supabase.rpc(
      "review_session_access_request",
      {
        ...
        p_capability: input.decision === "approved" ? "commenter" : null,
      },
    );
    if (error) return unavailableOrError(error.code);
    if (!Array.isArray(data) || data.length === 0) {
