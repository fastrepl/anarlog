...
|-------------|-----------|-------------|
| dev | `com.char.dev` | `com.char.dev` |
| staging | `com.char.staging` | `com.char.staging` |
| stable | `com.char.stable` | `char` |
| nightly | `com.char.nightly` | `char` |

Stable and nightly share the same folder (`char`) in production builds.

#### vault_base

Priority**:**
1. `VAULT_BASE` env var (highest)
2. `vault_path` in `char.json` (at global_base)
3. Fall back to `global_base`

Example:
```bash
VAULT_BASE="$HOME/Library/Application Support/char" pnpm -F desktop tauri:dev
```
...
|----------|-------------|
| `char.json` | Global config (vault_path, etc.) |
