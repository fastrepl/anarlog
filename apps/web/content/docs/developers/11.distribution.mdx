...
flatpak-builder --user --install --force-clean \
  flatpak-build-dir \
  apps/desktop/flatpak/com.char.Char.yml
```
...
flatpak run --devel --command=sh com.char.Char
...
- [ ] The app runs correctly via `flatpak run com.char.Char`
...
4. Create the app directory and copy files:
   ...
   ```bash
   mkdir com.char.Char
   cp /path/to/char/apps/desktop/flatpak/com.char.Char.yml com.char.Char/
   cp /path/to/char/apps/desktop/flatpak/com.char.Char.desktop com.char.Char/
   cp /path/to/char/apps/desktop/flatpak/com.char.Char.metainfo.xml com.char.Char/
   cp /path/to/char/apps/desktop/flatpak/pnpm-sources.json com.char.Char/
   cp /path/to/char/apps/desktop/flatpak/cargo-sources.json com.char.Char/
   ```
...
6. Commit and push:
   ...
   ```bash
   git add .
   git commit -m "Add com.char.Char"
   git push origin com.char.Char
   ```
...
- Flathub creates a dedicated repository: `flathub/com.char.Char`
...
1. Update the version in `com.char.Char.metainfo.xml`
...
apps/desktop/
  flatpak/
    com.char.Char.yml        # Flatpak manifest
    com.char.Char.desktop    # Desktop entry file
    com.char.Char.metainfo.xml  # AppStream metadata
