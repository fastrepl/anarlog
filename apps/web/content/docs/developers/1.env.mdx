...
OTEL_EXPORTER_OTLP_ENDPOINT=""         # optional, collector endpoint for API traces
HONEYCOMB_API_KEY=""                   # optional, direct Honeycomb export (if no collector)
HONEYCOMB_API_ENDPOINT=""              # optional, Honeycomb gRPC endpoint
HONEYCOMB_DATASET=""                   # optional, Honeycomb dataset name
HONEYCOMB_UI_BASE_URL=""               # optional, base URL for trace links in logs
HONEYCOMB_UI_TEAM=""                   # optional, Honeycomb team slug for trace links
HONEYCOMB_UI_ENVIRONMENT=""            # optional, Honeycomb environment for trace links
SUPABASE_URL="http://127.0.0.1:54321"
...
The observability variables are optional. If `OTEL_EXPORTER_OTLP_ENDPOINT` is set, traces are sent to an OTel collector. If `HONEYCOMB_API_KEY` is set instead, traces go directly to Honeycomb. The `HONEYCOMB_UI_*` variables enable clickable trace links in server logs.
