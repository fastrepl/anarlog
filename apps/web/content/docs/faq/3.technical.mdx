...
A CLI app (`apps/cli`) is in early development. It provides a TUI-based live transcription interface using the same `listener-core` engine as the desktop app. The CLI binary is named `char` and supports `auth` and `tui` subcommands. Recent improvements include theme support and various UX refinements.
...
## What export formats does Char support?

Char can export sessions as PDF, VTT (WebVTT subtitle format), and Org mode (`.org`). PDF export includes options for which sections to include (summary, transcript, memos). VTT exports the transcript with timestamps. Org mode produces an Emacs-compatible file.
