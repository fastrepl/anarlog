...
- `char.json` — vault configuration (custom vault path if set)
- `search/` — Tantivy full-text search index

On macOS, this is typically `~/Library/Application Support/char/`. On Linux, `~/.local/share/char/`.
...
- **Pro curated models:** Audio is proxied through `pro.char.com` (our server) and forwarded to a curated STT provider. The proxy does not store your audio.
...
- App version and platform information (`char-desktop@{version}`)
- The tag `service: "char-desktop"`
...
- Auth tokens to `pro.char.com` when using Pro features
