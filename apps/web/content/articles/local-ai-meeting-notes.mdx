...
This guide compares five tools by their documented processing and storage choices: what stays on your machine, what leaves, what the tradeoffs are, and what it costs. The Anarlog setup instructions below were checked against its documentation on September 16, 2026; they are not a cross-product accuracy benchmark.
...
## How to set up an offline Anarlog meeting

Local transcription and local summaries are separate choices. Selecting Ollama for summaries does not change where your audio is transcribed.

![Anarlog Settings → Transcription with the model dropdown open, showing on-device options](https://ijoptyyjrfqwaqhyxkxj.supabase.co/storage/v1/object/public/blog/library/products/anarlog/desktop/2026-09-23-settings-transcription-dropdown.png)

1. **Check model availability.** The built-in on-device transcription paths require a supported Apple Silicon Mac. Open **Settings → Transcription** and check the available models for your installed release. Apple Speech additionally requires macOS 26 and a supported language.
2. **Prepare speech-to-text.** Download a Soniqo model and select it under **Model being used**. Alternatively, configure Apple Speech's language assets or choose **On-device file** with a downloaded whisper.cpp `.bin`. Read the model's **Live** or **After recording** label; after-recording models do not produce a live transcript.
3. **Prepare summaries separately.** Download a text model in LM Studio, Ollama, or Unsloth, start its local server on the same computer, and select it in **Settings → Intelligence**. Choose a model that fits your available memory alongside the meeting app. Experimental Apple Intelligence is another option on eligible Macs running macOS 26 or later.
4. **Run a short offline check.** After all downloads finish, disconnect from the internet and record your own short sample. Verify capture, transcription, summary generation, and export. If a step fails, inspect its selected provider and the local server before relying on it in a meeting.
5. **Review optional data paths.** For a local-only workflow, leave CloudSync, sharing, Cloud API & Connectors, and hosted providers disabled. Avoid Chat's web search: it sends queries to a hosted endpoint even when both selected models are local.

The canonical meeting record is stored in local SQLite; recordings and attachments are separate local files. Markdown is an export format. Enabling a hosted AI provider sends the content needed for that operation to that provider. Optional encrypted CloudSync and the server-readable Cloud API & Connectors copy are distinct services; review each before enabling it.

Sources: Anarlog's [offline setup](https://docs.anarlog.so/offline), [model compatibility](https://docs.anarlog.so/models-and-providers), and [privacy policy](/privacy/). This is a setup procedure you can verify on your own hardware, not a claim that every model or operating system supports offline transcription.
...
- Transcription: choose an available on-device speech model or a hosted provider. Built-in on-device transcription currently requires a supported Apple Silicon Mac. Ollama and LM Studio serve text models for summaries, not this speech-to-text step.
...
- This offline setup describes the desktop app; do not assume the same models are available on another platform.
- Verify any export or integration separately: sending meeting content to another service changes the data boundary.
...
If both transcription and summarization use local models, yes. Anarlog needs both a downloaded on-device speech model on a supported Apple Silicon Mac and a local summary model such as Ollama; follow the setup steps above. Meetily with Parakeet + Ollama, Talat with its built-in LLM, and Screenpipe with local Whisper plus a local summary model can also work without an internet connection. Shadow's transcription works offline but the AI summary features need a connection.

### 4. What hardware do I need to run local transcription?

Requirements depend on the app and both selected models. Anarlog's built-in local transcription currently requires a supported Apple Silicon Mac; Apple Speech requires macOS 26 and supported language assets. Local summary models have their own memory requirements. Check the [current model compatibility list](https://docs.anarlog.so/models-and-providers), leave memory for your meeting app, and test a representative recording. There is no single RAM minimum that guarantees every Whisper or summary model will run well.
