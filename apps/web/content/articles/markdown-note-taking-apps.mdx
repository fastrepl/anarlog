...
- 7,600+ community plugins - Extend Obsidian into basically whatever you need it to be
...
- The DB version's new sync approach (RTC, for Real-Time Collaboration) and its new mobile app are both still in alpha, a step behind the DB version's own beta status - expect rough edges if you depend on either
...
**[MarkText](https://github.com/marktext/marktext)** has one of the cleanest writing experiences in this list - distraction-free, real-time preview, good theme selection. Development had a long quiet stretch, but it picked back up: recent releases are shipping regularly again with active bug fixes and community pull requests. Worth trying if you need a focused writing editor, but it's still a smaller project than Obsidian or Logseq if you're planning to bet years of notes on it.
