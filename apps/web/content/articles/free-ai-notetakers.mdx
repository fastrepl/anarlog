...
Anarlog is free forever for local transcription, BYOK, and all core features. For a managed cloud option (with sync, integrations, and the easiest setup), see [Char](https://char.com). Enterprise needs (on-prem, SSO, custom branding, compliance) are also handled via Char.
