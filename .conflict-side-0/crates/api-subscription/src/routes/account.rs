        ...
        Err(e) => {
            ...
            return Err((
                ...
                Json(DeleteAccountResponse {
                    deleted: false,
                    pending: false,
                    error: Some("stripe_customer_lookup_failed".to_string()),
                }),
            )
                .into_response());
        }
        ...
        .await
    {
        ...
        Err(stripe::StripeError::Stripe(_, 404)) => {
            tracing::info!(
                enduser.id = %user_id,
                hyprnote.billing.customer.id = %customer_id,
                "stripe_customer_already_deleted"
            );
            Ok(())
        }
        Err(e) => {
...
mod tests {
    ...
    async fn mount_no_stripe_customer(server: &MockServer) {
    ...
    async fn accepts_deletion_only_after_the_durable_handoff() {
        ...
        mount_no_stripe_customer(&server).await;
        ...
        assert_eq!(requests[0].url.path(), "/rest/v1/profiles");
        assert_eq!(
            requests[1].url.path(),
            "/rest/v1/rpc/begin_account_deletion"
        );
        assert_eq!(requests[2].url.path(), "/auth/v1/user");
        assert_eq!(requests[3].url.path(), "/auth/v1/logout");
        assert!(
            requests
                .iter()
    ...
    async fn does_not_report_acceptance_when_the_durable_handoff_fails() {
        ...
        assert!(
            server
                ...
                .unwrap()
                .iter()
                .all(|request| request.url.path() != "/auth/v1/user")
        );
    }
