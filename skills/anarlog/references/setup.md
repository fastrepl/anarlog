...
The BlackMushi plugin connects to the hosted server:

```text
https://api.anarlog.so/mcp
```

The account needs BlackMushi Pro. In the desktop app, open **Settings → Developers → Cloud API & Connectors**, review the disclosure, and enable it. Wait for meeting snapshots to upload. The first tool use opens BlackMushi sign-in and consent. No cloud API key or local CLI is required for Cloud reads.
...
Open **BlackMushi → Settings → Developers** and select **Install**. Direct-download builds install:

- macOS and Linux (DEB / AppImage): `~/.local/bin/anarlog`
- Windows: `%LOCALAPPDATA%\BlackMushi\bin\anarlog.exe`
...
Run the BlackMushi desktop app once so its local database exists. The CLI works while the app is closed after that.

Cloud sign-in does not require a graphical session on the BlackMushi machine. Run `anarlog auth login`, open the printed URL in any browser, and paste the copied callback URL into the CLI prompt. Confirm the session with `anarlog auth status`. With `--json`, the login URL is printed to stderr and the callback is read from stdin.
...
Use `--db-path FILE` or `ANARLOG_DB_PATH` only when the database is outside BlackMushi's default application-data location.
