...
- Canonical project skills live in `.agents/skills/`. `.cursor/skills/` and `.claude/skills/` are discovery shims so Cursor Cloud and Claude Code pick them up; edit the `.agents` copies only. `qa-critical-ux` is macOS-native and is not a cloud-environment gate.
