...
description: "Choose the right BlackMushi build for macOS, Windows, or Linux."
---

[Download BlackMushi](https://anarlog.so/download) to get the latest stable desktop release. The download page offers every supported platform from the same release.
...
2. Open the DMG and move BlackMushi to **Applications**.
3. Open BlackMushi and follow the setup screen.
4. Allow microphone, system audio, and calendar access when you want to use those features.

## Windows

1. Download the **Windows x64** installer or [get BlackMushi from Microsoft Store](https://apps.microsoft.com/detail/XPDLN196NKSW10).
...
3. Open BlackMushi and complete setup.
...
On Debian, Ubuntu, or a compatible distribution, add BlackMushi's signed repository to get updates through APT:
...
APT verifies the repository metadata with the dedicated BlackMushi archive key. New stable releases become available through the normal `apt upgrade` flow.
...
- Apple Calendar integration is available on macOS. Google Calendar works through your BlackMushi account on every supported platform.
- Soniqo, Apple Speech, and On-device file transcription require Apple Silicon. Apple Speech also appears only when a compatible Mac running macOS 26 reports the required system assets. On other systems, choose BlackMushi Cloud or configure a supported transcription provider. See [how to use on-device models](/help#use-on-device-models) and [local models for hour-long calls](/help#local-model-hour-long-calls).
- Windows and Linux support are in beta. Include your operating system and BlackMushi version when you report a platform-specific problem.
- BlackMushi does not currently offer a mobile app.
...
If you already use BlackMushi Nightly, keep these compatibility notes in mind before
switching to stable.
...
separate per app. Quit one app before opening the other: BlackMushi does not open
...
stable changelogs appear on the BlackMushi website. If you install the bundled CLI
