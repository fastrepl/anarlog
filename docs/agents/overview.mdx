---
title: "Agent overview"
description: "Give an agent useful BlackMushi context without exposing storage internals or unbounded transcripts."
---

BlackMushi gives agents three ways to access meeting context. The BlackMushi plugin uses hosted Cloud MCP. Use the local CLI when Cloud is missing a meeting. Local CLI and MCP reads are bounded; both can also stage proposals for human review. Hosted Cloud API and remote MCP are read-only.

| Transport | Choose it when | Output |
| --- | --- | --- |
| Cloud MCP | The default BlackMushi plugin, or any remote agent with OAuth | Hosted snapshots; requires BlackMushi Pro and explicit opt-in |
| CLI | Cloud has no snapshot, or the agent only has shell access | Local or authenticated Cloud reads; JSON with `--json`, or Markdown for people |
| Local MCP | A fully local agent that should not use Cloud | Structured tool results and contextual resources |

## Recommended agent flow

1. List recent meetings on Cloud MCP, or search by a short title fragment.
2. If Cloud has no match, search again with the local CLI.
3. Use a returned meeting ID. Never infer or invent one.
4. Fetch the meeting detail before requesting transcript text.
5. Fetch recurring history only when earlier meetings are relevant.
6. Read transcript pages only until the task has enough evidence.

Prefer connected Cloud MCP because each tool carries its input schema. Fill gaps with the CLI. Optional local MCP is for agents that stay fully on-device.

## Safety boundary

Agents should use an BlackMushi interface, not SQLite. The supported interfaces apply BlackMushi's rules for canonical notes, generated summaries, excluded participants, recurring meetings, and transcript text.

Meeting reads stay bounded. An agent cannot apply a note or summary change. It can stage a proposal with CLI `proposals` commands or the local MCP `propose_*` tools. A human reviews the unified diff in the desktop app and applies or declines it. Hosted Cloud API and Cloud MCP stay read-only. CLI export can create a separate file; replacing a file requires `--force`.

Local CLI and MCP access stays on your computer. Cloud access uploads a separate server-readable copy of your meeting data. Review the [Cloud API disclosure](/reference/api-cloud) before enabling it.

<CardGroup cols={2}>
  <Card title="CLI for agents" icon="terminal" href="/agents/cli">
    Use JSON output and resolve meeting IDs deliberately.
  </Card>
  <Card title="MCP for agents" icon="plug" href="/agents/mcp">
    Connect Cloud MCP through the BlackMushi plugin, or run local stdio as an option.
  </Card>
  <Card title="Cloud API and remote MCP" icon="cloud" href="/reference/api-cloud">
    Enable hosted access and connect with OAuth or a connector key.
  </Card>
  <Card title="Webhooks" icon="webhook" href="/reference/webhooks">
    Receive signed meeting events at your endpoint.
  </Card>
</CardGroup>
