...
description: "Connect an MCP client to BlackMushi meeting context over hosted OAuth or local stdio."
---

The BlackMushi plugin connects to hosted Cloud MCP at `https://api.anarlog.so/mcp` with OAuth. That is the default for Claude, Cursor, ChatGPT, Codex, and Copilot.
...
Enable **Cloud API & Connectors**, then install the **BlackMushi** plugin. The host discovers OAuth from the MCP endpoint. See [Cloud API and connectors](/reference/api-cloud#remote-mcp).
...
The local server communicates over standard input and output. It opens no port and sends no meeting data to BlackMushi's servers.
