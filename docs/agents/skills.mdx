---
title: "BlackMushi skill"
description: "Teach a compatible agent when and how to use BlackMushi's MCP and CLI surfaces."
---

The public BlackMushi skill teaches agents a single workflow across Cloud MCP and the local CLI:

1. Call connected Cloud MCP tools first.
2. Fill gaps with `anarlog --json` when Cloud has no snapshot.
3. Resolve meeting IDs from search results.
4. Fetch meeting detail before transcript text.
5. Keep transcript reads bounded and never access SQLite directly.

## Sources

- Published skill: `https://docs.anarlog.so/skill.md`
- Repository package: [`skills/anarlog`](https://github.com/fastrepl/anarlog/tree/main/skills/anarlog)
- Agent plugin: [`agent-plugins/anarlog`](https://github.com/fastrepl/anarlog/tree/main/agent-plugins/anarlog)

The published file is self-contained. The repository package adds references for setup, CLI commands, MCP tools, and errors.

## Install the plugin

The `BlackMushi` plugin connects to hosted Cloud MCP and bundles the skill. Enable **Settings → Developers → Cloud API & Connectors** first. [Install the BlackMushi CLI](/installation) when the skill needs to fill a meeting that is not in Cloud.

<Note>
  If you previously installed **BlackMushi Cloud**, replace it with **BlackMushi**.
</Note>

For Claude Code:

```bash
claude plugin marketplace add fastrepl/anarlog
claude plugin install anarlog@fastrepl
```

For GitHub Copilot CLI:

```bash
copilot plugin marketplace add fastrepl/anarlog
copilot plugin install anarlog@fastrepl
```

For ChatGPT and Codex, add the repository source, restart the ChatGPT desktop app, then install **BlackMushi** from the Fastrepl source in the Plugins Directory:

```bash
codex plugin marketplace add fastrepl/anarlog \
  --sparse .agents/plugins \
  --sparse agent-plugins/anarlog
```

The first cloud tool use opens BlackMushi's OAuth sign-in and consent flow; no cloud API key is required for Cloud reads.

Cursor Teams and Enterprise administrators can import `https://github.com/fastrepl/anarlog` as a team marketplace, then install **BlackMushi**.

## Install in an agent

Install it from the documentation site with the skills CLI:

```bash
npx skills add https://docs.anarlog.so
```

For a filesystem-based agent, copy the complete `skills/anarlog` directory into that agent's skills directory. Keep the directory name `anarlog` so relative reference links continue to work.

<Note>
  Agent skill locations and installer commands vary by client. Follow your agent's skill installation instructions rather than copying only `SKILL.md`.
</Note>

The contributor skills under `.agents/skills` are internal repository workflows. They are not part of the public BlackMushi skill.
