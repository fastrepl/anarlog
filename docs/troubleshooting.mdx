...
- **Microphone** is allowed so BlackMushi can hear you.
- **System Audio** is allowed so BlackMushi can hear the other people on the call.
...
A summary needs an Intelligence model. Open **Settings → Intelligence**, then select BlackMushi Cloud or configure another provider.
...
Open **Settings → Meetings** and check **Audio file retention** under **Audio**. BlackMushi can delete audio immediately or after a selected period while keeping the note and transcript.
...
BlackMushi lists supported meeting assistants it detects on your computer. Open the source app, restart BlackMushi, and return to **Settings → Imports**.
...
Sharing requires BlackMushi Pro, a signed-in account, an internet connection, and a generated summary. If a recipient cannot open the note, check their invitation and **General access** under **Share**.
...
Include your operating system, BlackMushi version, the exact error, what you expected, and the shortest steps that reproduce the problem. For AI failures, include the selected provider and model.
