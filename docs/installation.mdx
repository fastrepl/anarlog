...
description: "Install the current BlackMushi CLI and point it at local meeting data."
---
...
| Windows | `anarlog` | `%LOCALAPPDATA%\BlackMushi\bin\anarlog.exe` (added to your user PATH) |
...
By default, the CLI looks for `anarlog/app.db` in your operating system's application-data directory. It also recognizes legacy BlackMushi locations.
...
If you have an BlackMushi data directory containing `app.db`, use:
