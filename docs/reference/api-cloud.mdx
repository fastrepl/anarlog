...
Add `https://api.anarlog.so/mcp` as an MCP connection in your Instinct account and name it **Anarlog**. Complete the Anarlog sign-in and consent flow with the account that has Cloud API & Connectors enabled. Do not paste an API key.
...
https://api.anarlog.so/v1 and the OpenAPI document is
https://api.anarlog.so/openapi.json. Authenticate every request with the
header "Authorization: Bearer <my key>". It is read-only: list meetings,
read a meeting, read its transcript, and export it.
```
